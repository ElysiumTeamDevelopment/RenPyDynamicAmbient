init python:
    ...
    class DynamicAmbientSystem:
        ...
        def __getstate__(self):
            ...
            # Save only essential state that can be safely pickled
            # Avoid any threading objects completely
            safe_state = {
                ...
                'ambient_channels': list(self.ambient_channels),
                'ambient_start_time': self.ambient_start_time,
                'last_update_time': self.last_update_time,
                'global_cooldown_end_time': self.global_cooldown_end_time,
                'minimum_rest_time': self.minimum_rest_time,
                'maximum_rest_time': self.maximum_rest_time,
                'elevated_tracks_count': self.elevated_tracks_count,
                'min_tracks_per_wave': self.min_tracks_per_wave,
                'max_tracks_per_wave': self.max_tracks_per_wave,
                'current_wave_limit': self.current_wave_limit,
                'wave_elevation_count': self.wave_elevation_count,
                'initial_cooldown_time': self.initial_cooldown_time,
                'tracks_fading_out': set(self.tracks_fading_out),
                'active_arrangement_name': self.active_arrangement.name if self.active_arrangement else None,
                'active_layers': set(self.active_layers),
            }
            
            # Deep copy tracks without any threading objects
            safe_tracks = {}
            for track_id, track_data in self.tracks.items():
                safe_tracks[track_id] = {k: v for k, v in track_data.items() 
                                        if not isinstance(v, (threading.Timer, threading.Thread))}
            safe_state['tracks'] = safe_tracks
            
            # Deep copy track_states
            safe_state['track_states'] = copy.deepcopy(self.track_states)
            
            # Deep copy main_theme (no threading objects there)
            safe_state['main_theme'] = copy.deepcopy(self.main_theme)
        ...
        def __setstate__(self, state):
            ...
            for key in ['is_active', 'is_fading_out', 'is_main_menu', 'base_volume', 
                        'fade_duration', 'minimum_volume', 'ambient_channels',
                        'ambient_start_time', 'last_update_time', 'global_cooldown_end_time',
                        'minimum_rest_time', 'maximum_rest_time', 'elevated_tracks_count',
                        'min_tracks_per_wave', 'max_tracks_per_wave', 'current_wave_limit',
                        'wave_elevation_count', 'initial_cooldown_time', 'tracks_fading_out',
                        'active_layers', 'tracks', 'track_states', 'main_theme']:
                if key in state:
                    setattr(self, key, state[key])
            
            # Re-initialize timer containers (always empty after load)
            self.active_timers = []
            self.track_timers = {}
            self.arrangement_timer = None
            self.main_theme_timer = None
            
            # Reload config to restore arrangements and audio_assets
            self.arrangements = {}
            self.audio_assets = {}
            self.load_config()
            
            # Restore active arrangement by name
            active_arr_name = state.get('active_arrangement_name')
            if active_arr_name and active_arr_name in self.arrangements:
                self.active_arrangement = self.arrangements[active_arr_name]
            else:
...
# Threading objects (Timer) contain _thread.lock which cannot be pickled
...
init python:
    # Callback to save ambient state before save
    def _ambient_before_save():
        global _ambient_save_state
        if ambient and ambient.is_active:
            _ambient_save_state = {
                'is_active': ambient.is_active,
    ...
    # Callback to restore ambient state after load
    def _ambient_after_load():
        ...
        if _ambient_save_state:
            ...
            if arr_name:
                ...
                for layer in _ambient_save_state.get('active_layers', set()):
                    ambient.set_layer(layer, True)
    
    # Register callbacks
    config.save_json_callbacks.append(lambda d: _ambient_before_save())
    config.after_load_callbacks.append(_ambient_after_load)
