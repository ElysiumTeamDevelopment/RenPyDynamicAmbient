...
init python:
    # Переменная для хранения состояния mute
    if not hasattr(store, 'custom_is_muted'):
        store.custom_is_muted = False
    
    # Словарь для хранения предыдущих значений громкости
    if not hasattr(store, 'previous_volumes'):
        store.previous_volumes = {}

    def toggle_mute():
        store.custom_is_muted = not store.custom_is_muted
        
        # Список всех каналов, которые мы хотим заглушить
        # Включая стандартные и наши кастомные
        all_channels = ["music", "sound", "voice", "main_theme"] + ambient.ambient_channels

        if store.custom_is_muted:
            # Сохраняем текущую громкость и ставим на 0
            for channel in all_channels:
                store.previous_volumes[channel] = renpy.music.get_volume(channel=channel)
                renpy.music.set_volume(0.0, channel=channel)
        else:
            # Восстанавливаем предыдущую громкость
            for channel in all_channels:
                # Проверяем, есть ли сохраненное значение
                if channel in store.previous_volumes:
                    renpy.music.set_volume(store.previous_volumes[channel], channel=channel)
